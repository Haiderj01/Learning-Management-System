...
const HomeCourses = () => {
        ...
        fetch(`${API_BASE}/api/course/public?home=true&limit=8`)
            .then(async (res) => {
                if (!res.ok) {
                    ...
                    throw new Error(text || "Failed to fetch the courses from server");
                }
                return res.json();
            }
            )
    ...
    const handleBrowserClick = () => {
        ...
        if (!token) {
            toast.error("Please login to access this courses", {
                position: "top-right",
                transition: Slide,
                autoClose: 3000,
                theme: "dark",
                        ...
                        return (
                            ...
                            >
                                <div className={homeCoursesStyles.imageContainer}>
                                    <img src={course.image} alt={course.name} className={homeCoursesStyles.courseImage}
                                        loading="lazy" />
                                </div>
